...
class CLCRec(torch.nn.Module):
    def __init__(
        ...
        self.att_weight_1 = nn.Parameter(
            nn.init.kaiming_normal_(torch.rand((dim_E, dim_E)))
        )
    ...
    def forward(self, user_tensor, item_tensor):
        ...
        pos_item_tensor = (
            pos_item_tensor.repeat(1, 1 + self.num_neg).view(-1, 1).squeeze()
        )
        ...
        self.contrastive_loss_1 = self.loss_contrastive(
            head_embed, head_feat, self.temp_value
        )
        self.contrastive_loss_2 = self.loss_contrastive(
            user_embedding, all_item_input, self.temp_value
        )
