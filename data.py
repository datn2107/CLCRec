...
def load_dataset(data_path, has_v=True, has_a=True, has_t=True):
    dataset = np.load(data_path + "/metadata.npy", allow_pickle=True)
    ...
    dataset["a_feat"] = np.load(data_path + "/a_features.npy") if has_a else None
    dataset["v_feat"] = np.load(data_path + "/v_features.npy") if has_v else None
...
def preprocess_data(dataset):
    def preprocess_interactions(interactions, n_user):
        return {
            interaction[0]: interaction[1] + n_user for interaction in interactions
        }

    dataset["warm_items"] = dataset["warm_items"] + dataset["n_users"]
    dataset["cold_items"] = dataset["cold_items"] + dataset["n_users"]

    dataset["train_data"] = preprocess_interactions(dataset["train_data"], dataset["n_users"])
    dataset["val_data"] = preprocess_interactions(dataset["val_data"], dataset["n_users"])
    ...
    dataset["test_data"] = preprocess_interactions(dataset["test_data"], dataset["n_users"])
...
class TrainingDataset(Dataset):
    def __init__(
        ...
        for user in range(num_user):
            self.user_neg_items[user] = list(
                self.all_set - set(self.user_item_dict[user])
            )
    ...
    def __getitem__(self, index):
        ...
        neg_item = random.sample(self.user_neg_items[user], self.num_neg)
